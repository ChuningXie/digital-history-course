...
* **`convertion_csv2txt.py`**: The script used to extract specific columns (Trial_ID, Year, and Trial_Text) from our master CSV dataset (old_bailey_all_thefts.csv) and convert them into individual text files for MALLET analysis. It uses the Trial ID and Year to name each document, and fills the file with the corresponding trial text.
